...
def eval_reall_at_N(ds_name, N, res, use_rel=True, use_zero_shot=False):
    if (ds_name == 'vrd'):
        ...
        if gt is not None:
            ...
            print("read gt mat format error\n")
    else:
        # Testing all images is quite slow.
        # num_imgs = 8995
        num_imgs = 5000
        # used vg val data evaluate
        if (use_zero_shot):
...
def eval_obj_img(gt_boxes, gt_cls, pred_boxes, pred_cls, gt_thr=0.5, return_flag=False):
    ...
    for ii in range(pred_boxes.shape[0]):
        ...
        for jj in range(gt_boxes.shape[0]):
            ...
            in_box = np.array(
                [max(bbox[0], gt_box[0]), max(bbox[1], gt_box[1]), min(bbox[2], gt_box[2]), min(bbox[3], gt_box[3])])
...
def eval_object_recognition_top_N(proposals_path):
    ...
    with open(proposals_path, 'rb') as fid:
        proposals = pickle.load(fid, encoding='iso-8859-1')
...
def accuracy(predicate, target):
    # predicate: nxm, target=nxm
    all = 0
    real = 0.0
    for i in range(len(predicate)):
        real += np.sum((predicate[i] == target[i]))
        all += np.size(target[i])

    return np.float32((real / all) * 100)
