...
def train_net(train_data_layer, net, epoch, args):
    ...
    for step in tqdm(range(int(epoch_num))):
        ...
        obj_score, rel_score = net(image_blob, boxes, rel_boxes, SpatialFea, classes, ix1, ix2, class_embed, args)
...
def train_res(train_data_layer, net, epoch, args):
    net.train()
    losses = AverageMeter()
    time1 = time.time()
    epoch_num = train_data_layer._num_instance/train_data_layer._batch_size
    for step in tqdm(range(int(epoch_num))):
        ...
        # loss = args.criterion(rel_score, target, rel_so_prior)
        losses.update(loss.item())
        loss.backward()
        args.optimizer.step()
        if step % args.print_freq == 0:
...
def test_pre_net(net, args):
    ...
    for step in tqdm(range(int(test_data_layer._num_instance))):
...
def test_pre_res(net, args):
    ...
    for step in tqdm(range(int(test_data_layer._num_instance))):
        ...
        if(test_data is None):
            ...
            obj_bboxes_cell.append(None)
            continue
        image_blob, boxes, rel_boxes, SpatialFea, classes, ix1, ix2, class_embed,ori_bboxes = test_data
        rlp_labels_im  = np.zeros((100, 3), dtype = np.float)
        tuple_confs_im = []
        sub_bboxes_im  = np.zeros((100, 4), dtype = np.float)
        obj_bboxes_im  = np.zeros((100, 4), dtype = np.float)
...
def test_rel_res(net, args):
    ...
    pos_num, loc_num, gt_num, pos_num / (pos_num + loc_num), pos_num / gt_num))
    print('CLS REL TEST r50:%f, r50_zs:%f, r100:%f, r100_zs:%f' % (rec_50, rec_50_zs, rec_100, rec_100_zs))
    time2 = time.time()
    print("TEST Time:%s" % (time.strftime('%H:%M:%S', time.gmtime(int(time2 - time1)))))
    return rec_50, rec_50_zs, rec_100, rec_100_zs