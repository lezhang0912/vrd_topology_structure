...
class VrdDataLayer_weight(object):
    ...
    def __init__(self, ds_name, stage, model_type=None, proposals_path=None):
        ...
        self.img_dir = '../data/vrd/%s_images' %stage

        path = '../data/%s/%s.pkl' % (ds_name, stage)
        print(path)
        ...
        if (self.stage == 'train'):
            ...
            if (proposals_path != None) and model_type == 'RANK_IM':
                with open(proposals_path, 'rb') as fid:
                    ...
                    # self._boxes = proposals['boxes']
                    # self._pred_cls = proposals['labels']
                    # self._pred_confs = proposals['scores']
            if (proposals_path != None) and model_type == 'Faster-RCNN':
                with open(proposals_path, 'rb') as fid:
                    rcnn_proposals = pickle.load(fid)
                    self._boxes = rcnn_proposals['boxes']
                    self._pred_cls = rcnn_proposals['labels']
                    self._pred_confs = rcnn_proposals['scores']
                with open("../data/vrd/imagePath.pkl", 'rb') as fid:
                    self.test_imglist = pickle.load(fid)
        ...
        with open('../data/%s/word_embed.pkl' % ds_name, 'rb') as fem:
            self._word_embed = pickle.load(fem)
        # 添加unknown词向量
        with open('../data/%s/unknown_embed.pkl' % ds_name, 'rb') as fun:
            self._unknown = pickle.load(fun)
    ...
    def forward_train_rank_im(self):
        ...
        # im_path = anno_img['img_path'][:-3] + 'jpg'
        ...
        if anno_img['flipped'] is True:
            data = {"image": im, "bboxes": anno_img['boxes'], "category_id": anno_img['classes']}
            transform = flip_image()
            augmented = transform(**data)
            im = augmented["image"]
            anno_img['boxes'] = np.array(augmented["bboxes"])
        ...
        class_list = anno_img['classes'].tolist()
    ...
    def forward_det(self):
        ...
        if (boxes_img is None or boxes_img.shape[0] < 2):
            self._cur += 1
            if (self._cur >= len(self._anno)):
                self._cur = 0
            return None
    ...
    def forward_det_rcnn(self):
        ...
        anno_img = self.test_imglist[self._cur]
        im_path = osp.join(self.img_dir, anno_img)
        im = cv2.imread(im_path)

        if (im is None):
            self._cur += 1
            if (self._cur >= len(self.test_imglist)):
                self._cur = 0
            return None
        im_path = osp.join(self.img_dir, anno_img)
        im = cv2.imread(im_path)

        if self._boxes[self._cur] is None:
            self._cur += 1
            if (self._cur >= len(self.test_imglist)):
                self._cur = 0
            return None
...
if __name__ == '__main__':
    vrd = VrdDataLayer_weight(ds_name='vrd', stage='test',model_type='Faster-RCNN', \
